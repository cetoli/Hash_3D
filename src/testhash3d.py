...
class MyTestCase(unittest.TestCase):
    ...
    def test_all_lines_win_hi_colour(self):
        for i in range(0, 27, 3):
            self.i.clean()
            self.i.update(i, [23], i + 1, [24], i + 2, [25])
            self.h.debug = "dx0_w"
            pontua = self._readall()
            self.assertEqual(pontua, 1, f"but was {pontua}")

    def test_all_columns_win_hi_colour(self):
        for i in range(0, 3, 9):
            for j in range(0, 3, 1):
                self.i.clean()
                self.i.update(i + j, [23], i + j + 3, [24], i + j + 6, [25])
                self.h.debug = "dx0_w"
                pontua = self._readall()
                self.assertEqual(pontua, 1, f"but was {pontua}")

    def test_all_z_columns_win_hi_colour(self):
        for i in range(0, 3, 1):
            for j in range(0, 9, 3):
                self.i.clean()
                self.i.update(i + j, [23], i + j + 9, [24], i + j + 18, [25])
                self.h.debug = "dx0_w"
                pontua = self._readall()
                self.assertEqual(pontua, 1, f"but was {pontua}")
